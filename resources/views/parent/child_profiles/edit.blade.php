...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
    <div class="bg-white shadow-md rounded-lg overflow-hidden" id="profile-card">
        <div class="bg-indigo-600 px-4 sm:px-6 py-3 sm:py-4" id="profile-header">
            <h1 class="text-lg sm:text-xl font-bold text-white">Edit Child Profile: {{ $childProfile->name }}</h1>
        </div>
        <div class="p-4 sm:p-6">
            <form action="{{ route('parent.child-profiles.update', $childProfile) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="flex justify-center mb-6">
                    <img src="{{ $childProfile->avatarUrl }}" alt="{{ $childProfile->name }}" class="w-24 h-24 sm:w-32 sm:h-32 rounded-full object-cover border-4 border-indigo-100 shadow-md" id="profile-image">
                </div>
                ...
                <div class="mt-4 sm:mt-6 mb-4 sm:mb-6">
                    ...
                    <input type="number" class="w-full sm:w-1/3 rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 @error('age') border-red-500 @enderror" 
                        id="age" name="age" value="{{ old('age', $childProfile->age) }}">
                    @error('age')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
...
<script>
    ...
    function updateProfileColor() {
        ...
        if (genderSelect.value === 'girl') {
            ...
            profileImage.className = 'w-24 h-24 sm:w-32 sm:h-32 rounded-full object-cover border-4 border-pink-100 shadow-md';
            ...
            profileImage.className = 'w-24 h-24 sm:w-32 sm:h-32 rounded-full object-cover border-4 border-indigo-100 shadow-md';
            ...
            profileImage.className = 'w-24 h-24 sm:w-32 sm:h-32 rounded-full object-cover border-4 border-purple-100 shadow-md';
