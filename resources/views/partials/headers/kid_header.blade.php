    ...
    <div class="flex items-center justify-between px-6 py-4">
        ...
        <div class="relative" x-data="{ open: false }">
            ...
            <div x-show="open" @click.away="open = false" class="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg py-2 z-50">
                ...
                <a href=" route('kid.space') " class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 transition-colors">
...
<div id="parent-auth-modal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 hidden">
    <div class="bg-white rounded-xl p-6 w-full max-w-md mx-4 shadow-2xl transform transition-all">
        ...
        <form id="parent-auth-form" action="{{ route('parent.space') }}" method="POST" class="space-y-5">
            @csrf
            <div>
                <label for="parent-password" class="block text-sm font-medium text-[#9B59B6] mb-2">Security Password</label>
                <div class="relative">
                    <input type="password" id="parent-password" name="parent_password" class="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:ring-2 focus:ring-[#9B59B6] focus:border-[#9B59B6] transition-colors pl-10" required>
